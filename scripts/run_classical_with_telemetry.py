#!/usr/bin/env python3
"""
Run Classical Ground Roll Controller with 3D Telemetry Viewer

Shows the correct ground roll behavior using classical GNC.

Author: Kushal Koirala (with Claude Code)
Date: January 2025
"""

import asyncio
import threading
import time
import sys
import math
from pathlib import Path
import numpy as np

sys.path.insert(0, str(Path(__file__).parent.parent))

from aida_sim.env.flight_env_cessna172 import Cessna172Env, StateIndex
from aida_sim.io.telemetry import telemetry_server
from classical_ground_roll_controller import GroundRollController

# Telemetry units
TELEMETRY_UNITS = "metric"

shared_state = {
    "position": [0, 0, 0],
    "quaternion": [1, 0, 0, 0],
    "velocity": [0, 0, 0],
    "rates": [0, 0, 0],
    "surfaces": [0, 0, 0],
    "throttle": 0.0,
    "soc": 1.0,
    "voltage": 12.0,
    "load_factor": 1.0,
    "units": TELEMETRY_UNITS,
    "model": "cessna172",
}


def euler_to_quaternion(roll, pitch, yaw):
    """Convert Euler angles to quaternion [w, x, y, z]."""
    cy = math.cos(yaw * 0.5)
    sy = math.sin(yaw * 0.5)
    cp = math.cos(pitch * 0.5)
    sp = math.sin(pitch * 0.5)
    cr = math.cos(roll * 0.5)
    sr = math.sin(roll * 0.5)

    w = cr * cp * cy + sr * sp * sy
    x = sr * cp * cy - cr * sp * sy
    y = cr * sp * cy + sr * cp * sy
    z = cr * cp * sy - sr * sp * cy

    return [w, x, y, z]


def update_telemetry(obs, action):
    """Update shared telemetry state."""
    x, y, z = obs[StateIndex.X], obs[StateIndex.Y], obs[StateIndex.Z]
    u, v, w = obs[StateIndex.U], obs[StateIndex.V], obs[StateIndex.W]
    phi, theta, psi = obs[StateIndex.PHI], obs[StateIndex.THETA], obs[StateIndex.PSI]
    p, q, r = obs[StateIndex.P], obs[StateIndex.Q], obs[StateIndex.R]

    # Transform position for viewer (shift so runway starts at 0)
    viewer_x = x + 500.0
    viewer_y = y
    viewer_z = -z  # NED to viewer

    shared_state["position"] = [float(viewer_x), float(viewer_y), float(viewer_z)]

    quat = euler_to_quaternion(phi, theta, psi)
    shared_state["quaternion"] = [float(q) for q in quat]
    shared_state["velocity"] = [float(u), float(v), float(w)]
    shared_state["rates"] = [float(p), float(q), float(r)]

    throttle_norm = (action[0] + 1.0) / 2.0
    shared_state["throttle"] = float(np.clip(throttle_norm, 0.0, 1.0))
    shared_state["surfaces"] = [
        float(np.clip(action[1], -1.0, 1.0)),
        float(np.clip(action[2], -1.0, 1.0)),
        float(np.clip(action[3], -1.0, 1.0))
    ]


def run_simulation(dt=0.02, max_steps=800):
    """Run classical controller simulation with telemetry."""
    print("\n" + "="*60)
    print("  CLASSICAL GROUND ROLL - TELEMETRY VIEWER")
    print("="*60)
    print("Open browser to http://localhost:8000")
    print("Press Ctrl+C to stop\n")

    env = Cessna172Env(task='ground_roll', dt=dt)
    controller = GroundRollController(v_rotate=env.V_ROTATE, runway_heading=env.runway_heading)

    try:
        episode = 0
        while True:
            episode += 1
            print(f"\n--- Episode {episode} ---")

            obs, info = env.reset()
            done = False
            steps = 0
            total_reward = 0

            while not done and steps < max_steps:
                # Get action from classical controller
                action = controller.compute_action(obs)

                # Step environment
                obs, reward, terminated, truncated, info = env.step(action)
                done = terminated or truncated
                total_reward += reward

                # Update telemetry
                update_telemetry(obs, action)

                # Print status every 2 seconds
                airspeed = np.sqrt(obs[3]**2 + obs[4]**2 + obs[5]**2)
                if steps % int(2.0 / dt) == 0:
                    print(f"  Step {steps:3d}: Speed={airspeed*1.944:5.1f} KIAS, "
                          f"Alt={-obs[2]:5.2f}m, Pitch={np.rad2deg(obs[7]):5.1f}deg")

                steps += 1
                time.sleep(dt)

            # Episode summary
            reason = info.get('termination_reason', 'max_steps')
            airspeed = np.sqrt(obs[3]**2 + obs[4]**2 + obs[5]**2)

            print(f"\nEpisode {episode} complete:")
            print(f"  Steps: {steps}")
            print(f"  Total Reward: {total_reward:.1f}")
            print(f"  Final Airspeed: {airspeed*1.944:.1f} KIAS ({airspeed:.1f} m/s)")
            print(f"  Termination: {reason}")

            if airspeed >= env.V_ROTATE:
                print("  SUCCESS: Reached rotation speed!")

            time.sleep(3.0)

    except KeyboardInterrupt:
        print("\n\nSimulation stopped by user")

    env.close()


async def main_async(dt, max_steps):
    """Run telemetry server and simulation concurrently."""
    server_task = asyncio.create_task(
        telemetry_server(lambda: shared_state, host="0.0.0.0", port=8765)
    )

    await asyncio.sleep(1)

    sim_thread = threading.Thread(
        target=run_simulation,
        args=(dt, max_steps),
        daemon=True
    )
    sim_thread.start()

    try:
        await server_task
    except KeyboardInterrupt:
        print("\nShutting down...")


def main():
    asyncio.run(main_async(0.02, 800))
    return 0


if __name__ == "__main__":
    sys.exit(main())
